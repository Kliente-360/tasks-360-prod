...
export function BriefingClient() {
  ...
  // Redistribuição: pessoas sobrecarregadas + top tasks clicáveis
    ...
    for (const p of wca.pessoas) {
      ...
      if (personTasks.length > 0) {
        out.push({
          ...
          semLabel,
          targetNome: folga ? folga.nome.split(' ')[0] : null,
          tasks: personTasks,
        });
      }
    ...
    <div className="flex flex-col gap-4 md:gap-6">
                  ...
                  return (
                    <div key={s.projetoId} className="grid gap-1 items-center" style={{ gridTemplateColumns: '150px repeat(4, 1fr) 60px' }}>
                      <div className="text-xs truncate pr-1">
                        <a
                          href={projetoHref(s.clienteId, s.projetoId)}
                          className="hover:underline"
                          title={`${cliNome} · ${s.nome}`}
                        >
              ...
              return (
                <div key={p.projetoId} className="px-3 md:px-4 py-3">
                  <div className="flex items-center justify-between mb-1.5">
                    <a
                      href={projetoHref(p.clienteId, p.projetoId)}
      ...
      <div className="bg-elev border border-line rounded-xl overflow-hidden">
        <div className="px-3 md:px-4 py-3 border-b border-line">
          ...
          <p className="text-[11px] text-muted mt-0.5">Sugestões automáticas — revisar antes de aplicar</p>
        </div>
              ...
              <div key={i} className="px-3 md:px-4 py-3">
                <div className="flex items-start gap-2 mb-2">
                  ...
                  <span className="text-sm text-ink leading-snug">
                    <span className="font-medium">{s.pessoaNome}</span>
                    {' '}em sobrecarga {s.semLabel}
                    {s.targetNome && (
