...
export function LoginForm() {
  ...
  return (
    <div>
      <div className="font-brand text-lg font-semibold mb-1">Entrar</div>
      <div className="text-xs text-muted mb-5">Acesso restrito a pessoas cadastradas.</div>

      {/* SEÇÃO 1 · Time interno (Google SSO) */}
      <div className="mb-5">
        <div className="text-[10px] uppercase tracking-wider text-muted font-mono mb-2.5">
          Time interno · Kliente 360
        </div>
        <button
      ...
      {/* SEÇÃO 2 · Cliente externo (magic link) */}
      <div className="border-t border-line pt-5">
        ...
        <input
          ...
          onKeyUp={(e) => e.key === 'Enter' && sendMagicLink()}
          disabled={sending}
        ...
        <button
          ...
          disabled={sending || !email.trim() || googleLoading}
        >
          {sending ? 'enviando…' : 'Receber código por email'}
        </button>
      </div>

      <div className="text-[11px] text-muted mt-5 leading-relaxed border-t border-line pt-4">
        Não está cadastrado? Peça acesso ao admin do espaço.
      </div>
    </div>
  );
}
