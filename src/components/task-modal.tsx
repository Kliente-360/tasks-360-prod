...
function TaskModal({ taskId, onClose }: { taskId: string | null; onClose: () => void }) {
  ...
  // ============ Mobile detail render (≤860px) ============
  // Reativa: matchMedia listener, re-render quando muda largura.
  ...
  useEffect(() => {
    ...
    const onChange = (e: MediaQueryListEvent) => setIsMobileDetail(e.matches);
  ...
  if (isMobileDetail) {
    const clienteNomeM = clientesById.get(editing.clienteId)?.nome ?? '—';
    const projetoNomeM = projetosById.get(editing.projetoId)?.nome ?? '—';
    const respNomeM = pessoasById.get(editing.pessoaId)?.nome ?? '—';
    const respIni = (respNomeM || '?').split(/\s+/).slice(0, 2).map((w) => w.charAt(0).toUpperCase()).join('');
    const prazoLbl = editing.prazo ? fmtDateShort(editing.prazo) : '—';
    const lateM = editing.prazo && editing.status !== 'concluido' && editing.prazo < new Date().toISOString().slice(0, 10);
    const toggleItem = (idx: number) => {
      const next = editing.checklist.slice();
    ...
    const saveLabel =
      ...
      saveState === 'error' ? 'falhou' :
      'salvo · agora há pouco';
    return (
      <div className="detail-bg" onPaste={onModalPaste}>
        <div className="detail-head">
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <h2>{editing.titulo || 'Nova tarefa'}</h2>
            <div className="save">
              <span className="d" />
              {saveLabel}
            </div>
          </div>
          <button type="button" className="x" onClick={close} aria-label="Fechar">
            ✕
          </button>
        </div>
        <div className="detail-body">
          <div className="dsec">
            <div className="t">Título</div>
            <input
          ...
          <div className="dsec">
            ...
            <div className="dfield">
              <span className="l">Responsável</span>
              <div className="v">
                <span
                  style={{
                ...
                >{respIni || '?'}</span>
                <span style={{ marginLeft: 8 }}>{respNomeM}</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <div className="dfield" style={{ flex: 1 }}>
                <span className="l">Prazo</span>
                <div
                  className="v mono"
                  style={lateM ? { color: 'var(--danger)' } : undefined}
                >{prazoLbl}</div>
              </div>
              <div className="dfield" style={{ flex: 1 }}>
                <span className="l">Esforço</span>
                <div className="v mono">{editing.esforco || 0}h</div>
              </div>
            </div>
            <div className="dfield">
              <span className="l">Prioridade / Status</span>
              <div className="v" style={{ gap: 8 }}>
                ...
                <span className="status" data-s={editing.status} style={{ fontSize: 12 }}>
                  <span className="status-dot" />
                  {lblStatus(editing.status)}
                </span>
              </div>
            </div>
          </div>
          {editing.checklist.length > 0 && (
            <div className="dsec">
              ...
              {editing.checklist.map((it, i) => (
                <div key={i} className={'dline' + (it.done ? ' done' : '')}>
                  <input
                    type="checkbox"
                    checked={it.done}
          ...
          {comments.length > 0 && (
            <div className="dsec">
              <div className="t">Comentários · {comments.length}</div>
              {comments.slice(0, 6).map((c) => (
                <div
                  key={c.id}
                  className={'dmsg' + (c.from_cliente ? ' cliente' : '')}
                >
                  <div className="mh">
                    <span
                      style={{
                    ...
                    >
                      {(c.author ?? '?').split(/\s+/).slice(0, 2).map((w) => w.charAt(0).toUpperCase()).join('') || '?'}
                    </span>
                    <span className="who">{c.author ?? '—'}</span>
                    <span className="when">{c.posted_em ? fmtPostedEm(c.posted_em) : ''}</span>
                  </div>
